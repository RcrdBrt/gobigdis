...
func (r *Reader) close() {
	utils.Debugf("Closing SST reader for %v", r.filename)
	if err := r.f.Close(); err != nil {
		utils.Debugf("Error while closing SST reader: %v", err)
	}
}
...
func (r *Reader) Find(ctx context.Context, key string) (value []byte, ts int64, err error) {
	...
	utils.Debugf("reading data block %v", bh)
...
func (r *Reader) findDataBlock(key string) (blockHandle, error) {
	...
	utils.Debugf("reading index block %v", r.indexBlockHandle)
...
func (r *Reader) readRawBlock(h blockHandle, fillCache bool) ([]byte, error) {
	utils.Debugf("reading raw block: %v", h)
	...
	if r.cache != nil {
		...
		if data := r.cache.Get(cacheKey); data != nil {
			utils.Debugf("cache hit for %v", h)
			return data, nil
		}
	}
	...
	if !verifyChecksum(bd, raw[h.size:]) {
		utils.Debugf("sst block corrupt, checksum mismatch. blockHandle: %v", h)
...
func (r *Reader) readFooter() error {
	if r.fLength < footerSize {
		utils.Debugf("sst file is too small to have footer. file: %v", r.filename)
	...
	if binary.LittleEndian.Uint64(footer[footerSize-8:]) != SstMagic {
		utils.Debugf("sst footer has invalid magic. file: %v", r.filename)
		return ErrCorruption
	}

	if !verifyChecksum(footer[:footerSize-12], footer[footerSize-12:footerSize-8]) {
		utils.Debugf("sst footer corrupted for %v", r.filename)
		return ErrCorruption
	}
...
func verifyChecksum(data []byte, sum []byte) bool {
	...
	if ec != c {
		utils.Debugf("crc got, want: %v %v", c, ec)
	}
	return ec == c
}
