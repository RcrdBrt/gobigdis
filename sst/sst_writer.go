...
func (s *Writer) writeChecksummedBlock(d []byte) error {
	...
	c := s.crc.Sum32()
	if err := utils.WriteUint32(s.w, c); err != nil {
		return err
	}
...
func (s *Writer) writeFooter(indexHandle blockHandle, filterHandle blockHandle) error {
	...
	c := s.crc.Sum32()
	utils.WriteUint32(footer, c)

	if err := utils.WriteUint64(footer, SstMagic); err != nil {
		return err
	}

	d := footer.Bytes()
	if len(d) != footerSize {
		utils.Debugf("writeFooter generated footer of wrong length: %v", d)
	}
