...
func (e *prefixEncoder) EncodeInto(w io.Writer, k []byte, offset uint32) error {
	...
	if err := utils.WriteUvarInt64(w, uint64(shared)); err != nil {
		return err
	}
	if err := utils.WriteUvarInt64(w, uint64(nonShared)); err != nil {
		return err
	}
	if nonShared > 0 {
...
func (e *prefixEncoder) WriteRestarts(w io.Writer) error {
	for _, r := range e.restarts {
		if err := utils.WriteUint32(w, uint32(r)); err != nil {
			return err
		}
	}
	if err := utils.WriteUint32(w, uint32(len(e.restarts))); err != nil {
		return err
	}
	return nil
}
...
func prefixDecodeFrom(r *bytes.Reader, lastKey []byte, scratch []byte) ([]byte, error) {
	...
	if uint64(len(lastKey)) < shared {
		utils.Debugf("decodeFrom: shared is %v, but lastKey is len %v", shared, len(lastKey))
